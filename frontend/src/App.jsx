...
export default function App() {
  ...
  const submitConfig = async () => {
    ...
    const res = await fetch("https://ironhalo-demo.onrender.com/config-strike", {
      method: "POST",
    ...
    setPatterns(data.pattern_detected);
    setRationale(data.rationale);
    setClause(`${data.clause_id}: ${data.clause_title}`);
    setDrift(data.drift_value);
    setIntent(data.intent_strength);
    setPerimeterIndex(data.perimeter_index);

    setForensics((prev) => [data, ...prev]);
    setLoading(false);
  };
  ...
  const replayEvent = (e) => {
    ...
    setPatterns(e.pattern_detected);
    setRationale(e.rationale);
    setClause(`${e.clause_id}: ${e.clause_title}`);
